...
if __name__ == '__main__':
    ...
    for _ in range(10):
        random_scale = [(0, 5), (6, 16), (17, 27), (28, 31)]
        # 格式 {层号x(str): 噪声比例p(float)}，x到x+1的activation的p占比的元素随机置零
        noise_ratio_dict = {}
        for i in range(4):
            layer = random.randint(*random_scale[i])
            noise_ratio_dict[str(layer)] = random.random() * 0.2 + 0.1
        handles = add_noise_hooks(model, noise_ratio_dict)

        torch.manual_seed(12546)
        result = compute_ppl(
            predictions=input_texts,
            model=model,
            tokenizer=tokenizer,
        ...
        print(f"partition_point: {list(noise_ratio_dict.keys())}")
        print(f"noise: {list(noise_ratio_dict.values())}")
        print(f"PPL: {result['mean_perplexity']}")
